...
#check COPYING files
LICSHA=`find . -name "COPYING" | grep -v android-sdk-linux | xargs sha512sum | egrep -o "[a-f0-9]+ " | sort -u`
LICMD5=`find . -name "COPYING" | grep -v android-sdk-linux | xargs    md5sum | egrep -o "[a-f0-9]+ " | sort -u`
